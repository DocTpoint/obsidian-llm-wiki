...
- **Branch is `fix/ux-b1-b2-b3-provider-statusbar-dedup`; PR #448 (open, 10 commits).** This entry is co-numbered with PR #447's v1.26.3 PATCH but ships on a separate branch because the bugs are UI/lint, not JSON-schema-pipeline. Two PRs landing in the same patch release is intentional — review surface stays narrow (each PR is independently reviewable). DocTpoint's CHANGES_REQUESTED review (`c8e50b2`) was answered with 5 follow-up commits (B1/B2/B3 CR fixes, Toast i18n, housekeeping).
- **No settings-schema change, no migration, no key rename; 21 new i18n keys** (18 B2.5 status-bar + 3 Toast) added across all 10 locales, plus the `fetchErrorNetwork` value change. i18n-parity guards (bidirectional + placeholder-drift + non-empty) pin all new content; existing fetch-flow regression guards pin the HTTP-status re-throw behaviour.

...
> **Status.** Work in progress for the next PATCH (target v1.26.3). Everything below except *Tracked* and *Planned* is merged on `main` since v1.26.2 (tag 2026-08-09) but not yet in a tagged release. Composition: see [ROADMAP v1.26.x PATCH track](./ROADMAP.md).

### Fixed

- **`openai-compat` SDK client dropped `response_format` before the wire (Issue #443, PR #447).** The destructure list at `openai-compat-sdk-client.ts:182` did not include `response_format`, so every LLM call site that asked for `{ type: 'json_object' }` got no server-side JSON constraint on openai-compat providers. The destructure now carries `response_format` end-to-end via a new `buildOutputArgs` helper (`src/llm-sdk/output-args.ts`) that translates to the AI SDK's `Output.object({ schema, name })` mechanism. **No-schema case is a no-op** — the helper returns `{}` for the no-schema path, restoring v1.14.0 `ca4a24d` behaviour exactly. DocTpoint's 2026-08-09 LM Studio / gemma-4-12b measurement (Issue #443 comment 1): `response_format: { type: 'json_object' }` answers HTTP 400 in 29 ms — shipping the no-schema arm to lmstudio / ollama / custom would have regressed #65. The schema-capable cohort (lmstudio / ollama / custom) gets `supportsStructuredOutputs: true` on `PREDEFINED_PROVIDERS` so the SDK encodes `json_schema` on the wire when a future caller opts in. The cloud cohort (openrouter / deepseek / kimi / glm / gemini / minimax) keeps `supportsStructuredOutputs: false`; for cloud callers the schema falls back to `json_object` (the SDK pushes a warning) — the per-caller migration PR must also flip the flag for that provider. **This PR closes the destructure bug + lays the schema-mode infrastructure — the schema arm is verified by wire-body unit tests only; no production caller exercises it in this PR.** The per-caller migration of the 16 sites to schema-mode follows the CLAUDE.md "one PR per call site" rule for #407 Stages 1+2 (one production caller per PR, gated on this PR validating in production). 3006 tests / 218 files (+8 vs v1.26.2's 2996). No behaviour change for openai / anthropic / codex paths (those go through their own SDK clients).

...
- **Lint dedup: cross-type pair filter (B3).** The dedup candidate generator emitted entity↔source and concept↔source pairs that shared a wiki subfolder bucket (`tp:` / `ic:` / `lh:`), polluting the LLM verify batch with nonsense questions. Per the #358 complementary memory model, a source mentioning an entity by name is NOT a duplicate of the entity — they live in different cognitive registers. Per the maintainer's 2026-08-12 direction, the dedup now considers only: entity↔entity, concept↔concept, entity↔concept, source↔source. Forbidden: entity↔source and concept↔source (in any order). New `pageTypeOf(path)` helper infers page type from `WIKI_SUBFOLDERS` segments; new `isCrossTypePairAllowed` guard rejects forbidden pairs at `addCandidate` injection time using canonicalized `smaller|larger` string keys (the first naive implementation used string ordering for canonicalization, which silently rejected the allowed entity↔concept combination — regression guard catches it).

### Notes for release engineering

- **Branch is `fix/ux-b1-b2-b3-provider-statusbar-dedup`; PR will be created after this commit lands.** This entry is co-numbered with PR #447's v1.26.3 PATCH but ships on a separate branch because the bugs are UI/lint, not JSON-schema-pipeline. Two PRs landing in the same patch release is intentional — review surface stays narrow (each PR is independently reviewable).
- **No settings-schema change, no migration, no i18n key rename.** `fetchErrorNetwork` is the only i18n key whose value changed (text content, not key name). 10-locale i18n-parity guard pins the new content; existing fetch-flow regression guards pin the HTTP-status re-throw behaviour.

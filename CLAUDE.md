...
**Last Updated:** 2026-08-01 (v1.25.12 PATCH on branch `feat/cli-ux-v1.25.12`, 6 commits ahead of main `c905ffd`, awaiting merge). v1.25.11 PATCH MERGED 2026-07-31 (last tagged release). v1.25.12 PATCH scope: 6 commits = CLI UX optimization for the headless ingest CLI from PR #372 — expose via `llm-wiki` bin + `pnpm llm-wiki` script (single tool entry, subcommand dispatch ready for future lint/query), rename `--thinking` → `--thinking-mode` (three-state enum `data-json | plugin-off | server-default`) and `--max-rounds` → `--round-base` (both throw deprecation pointing at v1.26.0), README Tools section in all 10 locales, subcommand dispatch + parseCliOptions extracted + numeric validation helpers with ~22 tests pinning the parser contract. Tool named `llm-wiki` (not `wiki-ingest`) per first-principles CLI design — one tool = one bin = subcommand dispatch; future lint/query hooks into the dispatch without rename.
